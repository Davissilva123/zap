-- ============================================================
-- ZapMenu — Correcao das metricas do painel Super-Admin
-- Execute no Supabase SQL Editor
-- Corrige: total_restaurants, in_trial, MRR excluindo super admins
-- ============================================================

-- -------------------------------------------------------
-- 1. get_mrr_stats corrigido
--    - Exclui super admins de todos os contadores
--    - total_restaurants exclui contas admin/super
-- -------------------------------------------------------
...
BEGIN
  ...
  -- IDs a excluir: super admins + membros da equipe admin
  SELECT ARRAY_AGG(DISTINCT u.id) INTO v_excluded_ids
  FROM (
    SELECT u2.id FROM super_admins sa JOIN auth.users u2 ON u2.email = sa.email
    UNION
    SELECT u3.id FROM admin_team_members atm JOIN auth.users u3 ON u3.email = atm.email
  ) u;
...
-- -------------------------------------------------------
-- 2. get_platform_stats corrigido
--    - total_restaurants exclui super admins e admins
-- -------------------------------------------------------
