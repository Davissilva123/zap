import {
  LayoutDashboard, UtensilsCrossed, Grid3X3, QrCode, Settings, LogOut, Receipt, Zap,
  ...
  MonitorPlay, ShoppingBag, BookOpen, Truck, FileBarChart, ChevronRight, MoreHorizontal, X,
} from 'lucide-react';
...
// 5 items shown in the mobile bottom nav (owner panel)
const MOBILE_NAV = [
...
export default function Layout() {
    ...
    (async () => {
      ...
      try {
        ...
        if (!resolvedPlan || resolvedPlan.status === 'none') {
          await Promise.resolve(supabase.rpc('create_trial_plan', { p_user_id: user.id })).catch(() => {});
  ...
  // Recheck plan status periodically so expired trials are caught without a full page reload
  useEffect(() => {
    if (!user || user.email === SUPER_ADMIN_EMAIL) return;
    const interval = setInterval(async () => {
      const plan = await db.getMyPlan().catch(() => null);
      if (!plan) return;
      if (plan.isBlocked || plan.status === 'expired' || plan.status === 'cancelled') {
        navigate('/planos');
        return;
      }
      if (plan.status === 'trial' && plan.trialEndsAt && new Date(plan.trialEndsAt) < new Date()) {
        navigate('/planos');
      }
  ...
  const renderItem = (item: NavItem) => {
    ...
    if (locked) {
      return (
        <button
          key={item.to}
          onClick={() => { navigate(`/upgrade?feature=${item.feature}`); setMoreOpen(false); }}
  ...
  return (
    <PlanContext.Provider value={{ planSlug, planName: planDisplayName || PLAN_DISPLAY[planSlug] || 'Básico' }}>
      <div className="min-h-screen bg-slate-100 flex">
        ...
        {/* "Mais" drawer — mobile only */}
        {moreOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex flex-col justify-end">
            <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setMoreOpen(false)} />
            <div className="relative bg-white rounded-t-3xl shadow-2xl max-h-[80dvh] flex flex-col overflow-hidden">
              <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-slate-100">
                <p className="font-bold text-slate-900 text-base">Todos os módulos</p>
                <button onClick={() => setMoreOpen(false)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <nav className="overflow-y-auto px-4 py-3 space-y-4 pb-6">
                {NAV_GROUPS.map(group => (
                  <div key={group.key}>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {group.items.map(item => {
                        ...
                        return (
                          <button
                            key={item.to}
                            onClick={() => { setMoreOpen(false); navigate(locked ? `/upgrade?feature=${item.feature}` : item.to); }}
                          ...
                          >
                            <div className="relative">
                              ...
                              {locked && <Lock className="w-2.5 h-2.5 absolute -top-1 -right-1 text-slate-400" />}
                            </div>
                ...
                <div className="pt-2 border-t border-slate-100 space-y-1">
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-50 text-emerald-700 text-sm font-semibold active:bg-emerald-100 transition-colors"
                  >
                    <Settings className="w-4 h-4" /> Meu plano
                  </button>
                  <button
                    onClick={handleLogout}
                    className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-500 text-sm font-semibold active:bg-red-50 transition-colors"
                  >
        ...
        <div className="flex-1 lg:ml-[230px] min-h-screen flex flex-col min-w-0 overflow-x-hidden">
          ...
          <header className="lg:hidden sticky top-0 z-30 bg-[#0d1117] border-b border-white/[0.06] px-4 h-12 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-white text-sm tracking-tight">ZapMenu</span>
            </div>
            {planStatus === 'trial_warning' && (
              <button
                onClick={() => navigate('/planos')}
                className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${daysRemaining <= 2 ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400'}`}
              >
          ...
          {/* Banner: período de teste — desktop only (mobile shows in header) */}
          {planStatus === 'trial_warning' && (
            <div className={`hidden lg:flex border-b px-4 py-2.5 items-center justify-between gap-3 flex-wrap ${daysRemaining <= 2 ? 'bg-red-50 border-red-200' : daysRemaining <= 5 ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-200'}`}>
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200 safe-area-bottom">
          <div className="flex">
            ...
            <button
              ...
              className={`flex-1 flex flex-col items-center gap-1 py-2 transition-colors ${moreOpen ? 'text-emerald-600' : 'text-slate-400'}`}
            >
              <MoreHorizontal className="w-[22px] h-[22px]" strokeWidth={moreOpen ? 2 : 1.75} />
