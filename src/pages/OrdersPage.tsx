...
import { Clock, CheckCircle, XCircle, Eye, X, Truck, ShoppingBag, MapPin, Inbox, MessageCircle, Loader2, Printer, Volume2, VolumeX, Ban, Star, LayoutGrid, Tag, Search, CalendarDays, Bike, Check, AlarmClock, ChefHat } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const doUpdateStatus = async (orderId: string, newStatus: string, driverName?: string, driverId?: string) => {
    ...
    if (whatsappConfigured && order && settings) {
      ...
      setWhatsappError('');
      try {
        const reviewSuffix = newStatus === 'COMPLETED' ? '?avaliar=1' : '';
      await sendWhatsAppNotification(settings.whatsappApiToken, settings.whatsappPhoneNumberId, { ...order, status: newStatus as Order['status'] }, settings.name, newStatus, settings.slug ? `${window.location.origin}/m/${settings.slug}/conta${reviewSuffix}` : undefined);
  ...
  const cancelOrder = async (orderId: string) => {
    ...
    if (order && ['PAID', 'PREPARING', 'READY', 'DELIVERING'].includes(order.status)) {
      await Promise.all(order.items.map(item =>
