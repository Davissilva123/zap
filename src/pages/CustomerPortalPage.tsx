            ...
            <div className="grid grid-cols-1 gap-3 mb-5">
              ...
              {(settings.cashbackPercent ?? 0) > 0 && (
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-black/5">
                  ...
                  {cashbackTotal >= 1 && settings.phone ? (
                    <a
                      href={`https://wa.me/55${settings.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Olá, ${settings.name}! 😊 Sou o(a) ${customerName} e tenho R$ ${cashbackTotal.toFixed(2).replace('.', ',')} de cashback acumulado. Gostaria de resgatar como desconto no meu próximo pedido!`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-emerald-500 text-white text-sm font-bold active:scale-[0.98] transition-transform"
                    >
                      ...
                      Solicitar resgate via WhatsApp
                    </a>
                  ) : cashbackTotal < 1 ? (
                    <p className="text-xs text-slate-400 text-center py-1">Faça mais pedidos para acumular cashback suficiente para resgatar (mín. R$ 1,00).</p>
                  ) : (
                    <p className="text-xs text-slate-400 text-center py-1">Entre em contato com o restaurante para resgatar seu cashback.</p>
                  )}
                </div>
              )}
            </div>
