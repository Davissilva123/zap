...
export default function PublicMenuPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5]">
          ...
          <div className="bg-white border-b border-slate-100 shadow-sm">
            <div className="max-w-xl mx-auto px-5 py-3 flex flex-wrap gap-2 items-center">
              {(() => {
                ...
                const hasHours = settings.openingHours && Object.keys(settings.openingHours).length > 0;
                return (
                  <>
                    ...
                    {hasHours && (
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-colors"
                      >
      ...
      {/* ── MODAL DE HORÁRIOS ── */}
      {showHours && settings?.openingHours && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowHours(false)}>
          <div className="bg-white w-full max-w-sm rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-900 text-base">Horário de funcionamento</h3>
              </div>
            ...
            <div className="px-5 py-4 space-y-1.5">
              {['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado'].map((day, i) => {
                ...
                return (
                  <div key={i} className={`flex items-center justify-between px-3 py-2.5 rounded-xl ${isToday ? 'bg-emerald-50 border border-emerald-100' : 'hover:bg-slate-50'}`}>
                    <span className={`text-sm font-semibold ${isToday ? 'text-emerald-700' : 'text-slate-600'}`}>
                      {day}{isToday ? <span className="ml-1.5 text-[10px] font-bold bg-emerald-500 text-white px-1.5 py-0.5 rounded-full">hoje</span> : ''}
                    </span>
            ...
            {settings.manualClosed && (
              <div className="px-5 pb-4">
                <p className="text-xs text-center text-red-500 font-medium bg-red-50 rounded-xl py-2.5">⚠️ Restaurante temporariamente fechado</p>
              </div>
            )}
          </div>
        </div>
