...
export default function PublicMenuPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    if (cart.length > 0) localStorage.setItem(`cart_${slug}`, JSON.stringify(cart));
    else localStorage.removeItem(`cart_${slug}`);
  }, [cart, slug]);
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5]">
      ...
      {settings.coverUrl ? (
        ...
        <div className="relative overflow-hidden" style={{ background: accent }}>
          ...
          <div className="relative max-w-xl mx-auto px-5 pt-14 pb-8">
            ...
            {(settings.address || settings.phone || publicReviews.length > 0 || settings.deliveryTime || (settings.openingHours && Object.keys(settings.openingHours).length > 0)) && (
              <div className="flex flex-wrap gap-2 mt-5">
                ...
                {(settings.address || settings.phone || (settings.openingHours && Object.keys(settings.openingHours).length > 0)) && (
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-white/20 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/20 hover:bg-white/30 transition-colors"
                  >
                    <Info className="w-3 h-3 text-white/80" />
                    <span className="text-white/90 text-xs font-semibold">Sobre o restaurante</span>
                  </button>
                )}
                {publicReviews.length > 0 && (
                  <button
      ...
      {/* ── MODAL SOBRE O RESTAURANTE ── */}
      {showHours && settings && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowHours(false)}>
          <div className="bg-white w-full max-w-sm rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <Info className="w-5 h-5 text-slate-400" />
                <h3 className="font-bold text-slate-900 text-base">Sobre o restaurante</h3>
              </div>
              <button onClick={() => setShowHours(false)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center">
            ...
            <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              {(() => {
                ...
                return (
                  <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${open ? 'bg-emerald-50 border border-emerald-100' : 'bg-red-50 border border-red-100'}`}>
                    <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${open ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
                    <span className={`text-sm font-bold ${open ? 'text-emerald-700' : 'text-red-600'}`}>
                      {open ? 'Aberto agora' : settings.manualClosed ? 'Fechado temporariamente' : 'Fechado no momento'}
                    </span>
                  </div>
                );
              })()}
              ...
              {settings.address && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Endereço</p>
                    <p className="text-sm text-slate-700 font-medium leading-relaxed">{settings.address}</p>
                  </div>
                </div>
              )}
              ...
              {settings.phone && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <a href={`tel:${settings.phone}`} className="text-sm font-semibold text-blue-600 hover:underline">
                      {settings.phone}
                    </a>
                  </div>
                </div>
              ...
              {settings.openingHours && Object.keys(settings.openingHours).length > 0 && (
                <div>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Horário de funcionamento</p>
                  <div className="space-y-1">
                    {['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado'].map((day, i) => {
                      const h = settings.openingHours[String(i)];
                      const isToday = new Date().getDay() === i;
                      return (
                        <div key={i} className={`flex items-center justify-between px-3 py-2 rounded-xl ${isToday ? 'bg-emerald-50 border border-emerald-100' : ''}`}>
                          <span className={`text-sm font-semibold ${isToday ? 'text-emerald-700' : 'text-slate-600'}`}>
                            {day}
                            {isToday && <span className="ml-1.5 text-[10px] font-bold bg-emerald-500 text-white px-1.5 py-0.5 rounded-full">hoje</span>}
                          </span>
                          <span className={`text-sm font-bold ${h?.open ? (isToday ? 'text-emerald-700' : 'text-slate-700') : 'text-slate-300'}`}>
                            {h?.open ? `${h.from} – ${h.to}` : 'Fechado'}
                          </span>
                        </div>
                      );
                    })}
              ...
              {step === 'cart' && cart.length > 0 && (
                <>
                  ...
                  {settings && !isRestaurantOpen(settings) ? (
                    <div className="w-full py-4 rounded-2xl bg-red-50 border border-red-100 text-center space-y-0.5">
                      <p className="text-red-600 font-bold text-sm">Restaurante fechado no momento</p>
                      <button onClick={() => setShowHours(true)} className="text-red-400 text-xs font-medium underline underline-offset-2">
