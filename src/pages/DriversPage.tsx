...
import { Bike, Plus, Edit2, Trash2, Phone, X, Check, ToggleLeft, ToggleRight } from 'lucide-react';
          ...
          {active.length > 0 && (
            <section>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {active.map(d => <DriverCard key={d.id} driver={d} onEdit={openEdit} onDelete={remove} onToggle={toggleActive} />)}
              </div>
            </section>
          )}
          {inactive.length > 0 && (
            <section>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {inactive.map(d => <DriverCard key={d.id} driver={d} onEdit={openEdit} onDelete={remove} onToggle={toggleActive} />)}
              </div>
            </section>
          )}
        </div>
...
function DriverCard({ driver, onEdit, onDelete, onToggle }: {
  driver: Driver;
  ...
  return (
    <div className={`bg-white rounded-2xl border p-4 flex items-start gap-3 transition-all ${driver.active ? 'border-slate-200 shadow-sm' : 'border-slate-100 opacity-60'}`}>
