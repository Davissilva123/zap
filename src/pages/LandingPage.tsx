    ...
    <div className="min-h-screen bg-white font-sans antialiased">
      ...
      {showDemo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setShowDemo(false)} />
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl z-10 max-h-[92vh] flex flex-col">

            {/* Header escuro com gradiente */}
            <div className="relative bg-gradient-to-br from-slate-900 via-slate-900 to-emerald-950 rounded-t-3xl p-7 flex-shrink-0 overflow-hidden">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_70%_80%_at_50%_-10%,rgba(5,150,105,0.35),transparent)]" />
              <button onClick={() => setShowDemo(false)} className="absolute top-4 right-4 p-2 rounded-xl bg-white/10 hover:bg-white/20 transition-colors text-white z-10">
                <X className="w-5 h-5" />
              </button>
              <div className="relative">
                <div className="inline-flex items-center gap-1.5 bg-emerald-500/20 border border-emerald-500/30 rounded-full px-3 py-1 text-emerald-400 text-xs font-semibold mb-4">
                  <Zap className="w-3 h-3" /> Sistema completo para restaurantes
                </div>
                <h2 className="text-2xl sm:text-3xl font-black text-white mb-2 leading-tight">
                  Tudo que seu restaurante precisa<br />
                  <span className="text-emerald-400">para vender mais todo dia</span>
                </h2>
                <p className="text-slate-400 text-sm max-w-xl mb-5">
                  Do cardápio digital ao controle de entregadores — veja por que centenas de restaurantes escolheram o ZapMenu para crescer.
                </p>
                <div className="flex gap-8">
                  {[
                    { n: '500+', l: 'Restaurantes ativos' },
                    { n: '98%',  l: 'Satisfação garantida' },
                    { n: '15min',l: 'Para configurar tudo' },
                  ].map((s, i) => (
                    <div key={i}>
            ...
            <div className="overflow-y-auto flex-1">
              ...
              <div className="p-6 border-b border-slate-100">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Problemas que resolvemos no seu dia a dia</p>
                <div className="grid sm:grid-cols-3 gap-3">
                  {[
                    {
                      icon: Smartphone, bg: 'bg-emerald-100 text-emerald-600', card: 'border-emerald-100 bg-emerald-50/50',
                      problem: 'Perde pedidos fora do horário',
                      solution: 'Cardápio online 24h, pedidos no piloto automático',
                    },
                    {
                      icon: CreditCard, bg: 'bg-blue-100 text-blue-600', card: 'border-blue-100 bg-blue-50/50',
                      problem: 'Maquininha consome seu lucro',
                      solution: 'PIX direto para sua conta, zero taxa de cartão',
                    },
                    {
                      icon: ChefHat, bg: 'bg-orange-100 text-orange-600', card: 'border-orange-100 bg-orange-50/50',
              ...
              {/* Funcionalidades */}
              <div className="p-6 border-b border-slate-100">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Tudo incluso no sistema</p>
                <div className="grid sm:grid-cols-2 gap-3">
                  {demoModules.map((m, i) => (
                    <div key={i} className="flex gap-3 p-3.5 rounded-2xl border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all">
              ...
              <div className="p-6 bg-slate-50 border-b border-slate-100">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">O que dizem nossos clientes</p>
                <div className="grid sm:grid-cols-2 gap-4">
                  {[
                    {
                      text: '"Triplicamos os pedidos no primeiro mês. Configurei em 20 minutos e o suporte me ajudou em tudo. Melhor investimento que fiz no restaurante."',
                      name: 'João S.', place: 'Pizzaria Bella Napoli — SP',
                    },
                    {
                      text: '"O PIX automático me economiza mais de R$800 por mês que eu pagava de taxa de maquininha. O sistema se paga sozinho."',
                      name: 'Maria L.', place: 'Restaurante Sabor & Arte — RJ',
                    },
                  ].map((t, i) => (
                    <div key={i} className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
                      ...
                      <p className="text-slate-700 text-sm italic leading-relaxed mb-3">{t.text}</p>
                      <div>
                        <p className="text-slate-900 text-xs font-bold">{t.name}</p>
                        <p className="text-slate-400 text-xs">{t.place}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Pricing strip */}
              <div className="p-6">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Planos acessíveis — sem fidelidade</p>
                <div className="grid grid-cols-3 gap-3">
                  {plans.map(p => (
                    <div key={p.slug} className={`rounded-2xl border-2 p-4 text-center ${p.highlight ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
                      {p.badge && (
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${p.highlight ? 'bg-emerald-200 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}>
                          {p.badge}
                        </span>
                      )}
                      <p className="font-black text-slate-900 text-sm mt-1.5 mb-1">{p.name}</p>
                      <p className="text-2xl font-black text-slate-900 leading-none">{p.price}</p>
                      <p className="text-xs text-slate-400 mt-0.5">/{p.period}</p>
                      {trialSettings[p.slug]?.enabled && (
                        <p className="text-xs text-emerald-600 font-semibold mt-1.5">
                          {trialSettings[p.slug].days} dias grátis
                        </p>
                      )}
                    </div>
                  ))}
                </div>
                <p className="text-center text-xs text-slate-400 mt-3">Cancele quando quiser · Sem multa · Sem fidelidade</p>
              </div>
            </div>

            {/* Footer CTA */}
            <div className="p-5 border-t border-slate-100 flex-shrink-0 bg-gradient-to-r from-emerald-50/60 to-white rounded-b-3xl">
              <div className="flex flex-col sm:flex-row items-center gap-3">
                ...
                <div className="flex gap-2.5 flex-shrink-0">
                  <button
                    onClick={() => window.open(waLink(), '_blank')}
                    className="flex items-center gap-2 px-4 py-3 bg-[#25D366] hover:bg-[#20bd5a] text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-green-500/20"
                  >
